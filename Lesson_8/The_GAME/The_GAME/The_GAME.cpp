...
Coord getAICoord(Field & s)
{
	...
	number = 0;
	if (s.ppfield[0][1] == Cell::EMPTY)
	{
		arr[number++] = { 0, 1 };
	}
	if (s.ppfield[1][0] == Cell::EMPTY)
	{
		arr[number++] = { 1, 0 };
	}
	if (s.ppfield[1][4] == Cell::EMPTY)
	{
		arr[number++] = { 1, 4 };
	}
	if (s.ppfield[4][1] == Cell::EMPTY)
	{
		arr[number++] = { 4, 1 };
	}	
	if (s.ppfield[0][3] == Cell::EMPTY)
	{
		arr[number++] = { 0, 3 };
	}
	if (s.ppfield[4][3] == Cell::EMPTY)
	{
		arr[number++] = { 4, 3 };
	}
	if (s.ppfield[2][0] == Cell::EMPTY)
	{
		arr[number++] = { 2, 0 };
	}
	if (s.ppfield[3][0] == Cell::EMPTY)
	{
		arr[number++] = { 3, 0 };
	}
	if (s.ppfield[0][2] == Cell::EMPTY)
	{
		arr[number++] = { 0, 2 };
	}
	if (s.ppfield[4][2] == Cell::EMPTY)
	{
		arr[number++] = { 4, 2 };
	}
	if (s.ppfield[3][4] == Cell::EMPTY)
	{
		arr[number++] = { 3, 4 };
	}
	if (s.ppfield[2][4] == Cell::EMPTY)
	{
		arr[number++] = { 2, 4 };
	}
	if (s.ppfield[3][3] == Cell::EMPTY)
	{
		arr[number++] = { 3, 3 };
	}
	if (s.ppfield[2][1] == Cell::EMPTY)
	{
		arr[number++] = { 2, 1 };
	}
	if (s.ppfield[3][1] == Cell::EMPTY)
	{
		arr[number++] = { 3, 1 };
	}
	if (s.ppfield[1][2] == Cell::EMPTY)
	{
		arr[number++] = { 1, 2 };
	}
	if (s.ppfield[1][3] == Cell::EMPTY)
	{
		arr[number++] = { 1, 3 };
	}
	if (s.ppfield[2][3] == Cell::EMPTY)
	{
		arr[number++] = { 2, 3 };
	}
	if (s.ppfield[1][1] == Cell::EMPTY)
	{
		arr[number++] = { 1, 1 };
	}
	if (s.ppfield[3][2] == Cell::EMPTY)
	{
		arr[number++] = { 3, 2 };
	}
...
void congratulations(GameProgress & progress)
{
	if (progress == GameProgress::WON_HUMAN)
	{
		int a;
		SetColor(12, 0),
		cout << "I failed just a battle. But not the war." << endl << "I WILL SHOW YOU FEAR!! ENTER ANY BOOTTON!!!" << endl;
		cin >> a;
		SetColor(2, 0);
